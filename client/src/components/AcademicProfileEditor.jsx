...
/** Program caps (mirror the server's). Additional majors also need admin approval. */
const MAX_MINORS = 2;
...
export default function AcademicProfileEditor({
  ...
  const isMinorMode = (mode) => mode === 'minor-add' || mode === 'minor-edit';

  /** The id the open picker is editing — null for the "add" modes. */
  function currentPickerId() {
    if (!picker) return null;
    if (picker.mode === 'primary')         return primaryMajorId;
    if (picker.mode === 'additional-edit') return additionalMajorIds[picker.index] ?? null;
    if (picker.mode === 'minor-edit')      return minorIds[picker.index] ?? null;
    return null;
  }
  ...
  function handlePickerChange(newValue, programOrPrograms) {
    ...
    // The picker already hides taken programs; this is the backstop. A slot may keep
    // its own current value, so only the *other* slots count as taken.
    const takenByOthers = (isMinorMode(mode) ? minorIds : [primaryMajorId, ...additionalMajorIds])
      .filter(Boolean)
      .filter((x) => x !== currentPickerId());
    if (id && takenByOthers.includes(id)) { setPicker(null); return; }
    ...
    if (mode === 'primary') {
      ...
      if (id && additionalMajorIds.length < MAX_ADDITIONAL_MAJORS) {
      ...
      if (id && minorIds.length < MAX_MINORS) {
    ...
    <>
          ...
          additionalMajorIds.length > 0
            ? additionalMajorIds.map((id, i) => (
                <ItemRow
                  ...
                  onAdd={i === additionalMajorIds.length - 1 && additionalMajorIds.length < MAX_ADDITIONAL_MAJORS ? () => setPicker({ mode: 'additional-add' }) : undefined}
                />
              ))
            : (
          ...
          {minorIds.length > 0
            ? minorIds.map((id, i) => (
                <ItemRow
                  ...
                  onAdd={i === minorIds.length - 1 && minorIds.length < MAX_MINORS ? () => setPicker({ mode: 'minor-add' }) : undefined}
                />
              ))
            : (
      ...
      {picker && (
        <AcademicProgramPicker
          type={isMinorMode(picker.mode) ? 'minor' : 'major'}
          value={currentPickerId()}
          multiple={false}
          // Programs already claimed by another slot — keeps the same major/minor
          // from being picked twice (the slot being edited stays selectable).
          excludeIds={
            (isMinorMode(picker.mode) ? minorIds : [primaryMajorId, ...additionalMajorIds]).filter(Boolean)
          }
