...
// Try the graph node's primary code then each match alias when looking up student status.
...
function CourseNode({ data }) {
  const { nodeId, code, title, creditHours, state, color, requirementType, requirementSubtype, highlight, matches, selectedOption, pickCount, requiredHours, requiredHoursLabel, alternatives,
    options, optionStates, activeOptionIdx, onCycle, bottleneckOption } = data;
  ...
  // alternative, all of its courses' prereq chains light up together.
  if (requirementType === 'choice' && requirementSubtype === 'exact_compound_choice' && alternatives?.length > 0) {
    ...
    return (
      <div style={{ opacity: dimmed ? 0.2 : 1, transition: 'opacity 0.15s' }} title={tipText}>
        ...
        <div style={{
          ...
          boxShadow: selected ? `0 0 0 3px ${cBorder}55, 0 2px 8px rgba(0,0,0,.15)` : `0 1px 4px rgba(0,0,0,.1)`,
        }}>
          {total > 1 && (
            <button
              type="button"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
            <span style={{ fontWeight: 700, fontSize: 11, color: cHead, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              Required — choose one
            </span>
          </div>
          {alternatives.map((alt, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: i < alternatives.length - 1 ? 3 : 0 }}>
                ...
                <div style={{
                  fontSize: 11,
                  color: isActive ? (activeInPlan ? '#b91c1c' : '#b45309') : '#374151',
                  fontWeight: isActive ? 700 : (alt.isBundle ? 600 : 400),
                  borderBottom: isActive ? `2px solid ${activeInPlan ? '#dc2626' : '#f59e0b'}` : '2px solid transparent',
                  paddingBottom: 1,
                }}>{alt.courses.join(' + ')}</div>
              </div>
            );
          })}
          {creditHours != null && (
            <div style={{ marginTop: 4, fontSize: 10, color: '#9ca3af' }}>{creditHours} hrs</div>
          )}
        </div>
  ...
  // Shorten common long labels for the compact node preview line.
  // Full labels are preserved in the tooltip (title attribute).
  ...
  // Opacity/scale driven by highlight role
  const dimmed    = highlight === 'dimmed';
  const selected  = highlight === 'selected';
  ...
  // Elective / UCC placeholder block — styled to match degree plan sheet
  if (isElective) {
    const pool = matches ?? [];
    const isPool = pool.length > 0;

    // Pool node: "Pick N of M" or "Select N hrs" with course/option list preview.
    if (isPool) {
      const isHoursBased = !!requiredHours;
      // Track/area pools are LOCKED degree requirements (not free electives) — emerald
      // "requirement pool" styling + a kind header so they read as part of the degree.
      ...
      return (
        <div style={wrapStyle} title={tipText}>
          ...
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 12, color: theme.head, marginBottom: 3 }}>
              {slotLabel}
            </div>
            <div style={{ fontSize: 10, color: theme.body, lineHeight: 1.4 }}>{preview}</div>
            {!isHoursBased && creditHours != null && (
              <div style={{ marginTop: 3, fontSize: 10, color: theme.dim, opacity: 0.7 }}>({creditHours} hrs)</div>
            )}
          </div>
          <Handle type="source" position={Position.Right} style={{ opacity: 0 }} />
        </div>
    ...
    return (
      <div style={wrapStyle} title={isUcc ? 'University Core Curriculum' : (title || undefined)}>
        ...
        <div style={{
          ...
          minWidth: 150, maxWidth: 210, textAlign: 'center',
          boxShadow: '0 1px 3px rgba(0,0,0,.06)',
        }}>
          {isUcc ? (
          ...
          {creditHours != null && (
            <div style={{ marginTop: 3, fontSize: 11, color: palette.text, opacity: 0.7 }}>({creditHours} hrs)</div>
          )}
        </div>
        <Handle type="source" position={Position.Right} style={{ opacity: 0 }} />
      </div>
  ...
  // numbered badge. Active option underlined RED if in the student's plan, AMBER if a
  // prospective preview. The in-plan option carries a green dot to distinguish it from the
  // option whose prereq chain is merely being previewed.
    ...
    return (
      <div style={wrapStyle} title={tip}>
        ...
        <div style={{ position: 'relative', background: bg, border, borderRadius: 8, padding: '8px 10px', minWidth: 150, maxWidth: 240, boxShadow, fontSize: 12, color: textColor }}>
          {/* nodrag/nopan + stopPropagation keep the click from being swallowed by
              React Flow's node drag/select handlers. */}
          <button
            type="button"
            className="nodrag nopan"
            onMouseDown={(e) => e.stopPropagation()}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}>
            <span style={{ fontWeight: 700, fontSize: 10, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Choose one</span>
          </div>
  ...
  return (
    <div style={wrapStyle}>
      <Handle type="target" position={Position.Left} style={{ opacity: 0 }} />
      <div style={{
        background: bg, border, borderRadius: 8, padding: '8px 10px',
        minWidth: 150, maxWidth: 210, boxShadow,
        ...
        {title && (
          <div title={title} style={{ marginTop: 2, lineHeight: 1.3, fontSize: 11, color: state === 'completed' ? '#4b7556' : '#374151', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {title}
          </div>
        )}
        <div style={{ marginTop: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {creditHours != null && (
            <span style={{ fontSize: 10, color: state === 'completed' ? '#4b7556' : '#9ca3af' }}>{creditHours} hrs</span>
          )}
          <span style={{ fontSize: 10, borderRadius: 4, padding: state === 'missing' ? 0 : '1px 5px', marginLeft: 'auto', background: state === 'missing' ? 'transparent' : badge.bg, color: badge.text }}>
            {badge.label}
          </span>
        </div>
      </div>
...
function ColumnHeaderNode({ data }) {
  return (
    <div style={{
      padding: '5px 12px', textAlign: 'center', minWidth: 150, maxWidth: 210,
      background: '#e5e7eb', borderRadius: 6, border: '1px solid #d1d5db',
      fontWeight: 700, fontSize: 12, color: '#374151', userSelect: 'none',
    }}>
      {data.label}
    </div>
  );
...
function PathOptionNode({ data }) {
  ...
  const boxShadow   = selected
    ? '0 0 0 3px #fde68a, 0 2px 8px rgba(0,0,0,.15)'
    : isBottleneck ? '0 0 0 3px #fecaca, 0 2px 8px rgba(0,0,0,.15)'
    : '0 1px 4px rgba(0,0,0,.1)';
  ...
  return (
    <div style={{ opacity: dimmed ? 0.2 : 1, transition: 'opacity 0.15s, box-shadow 0.15s' }}>
      <Handle type="target" position={Position.Left} style={{ opacity: 0 }} />
      <div style={{
        background: '#fff7ed', border: `${borderWidth}px solid ${borderColor}`,
        borderRadius: 8, minWidth: 170, maxWidth: 240, boxShadow, overflow: 'hidden',
      }}>
          ...
          <div className="nodrag nopan" style={{ display: 'flex', borderBottom: '1px solid #fed7aa' }}>
            {paths.map((path, idx) => (
              <button
                ...
                style={{
                  flex: 1, padding: '4px 4px', fontSize: 10, lineHeight: 1.3,
                  fontWeight: idx === activeIdx ? 700 : 400,
                  border: 'none', cursor: 'pointer',
                  background: idx === activeIdx ? '#fff7ed' : '#fef3c7',
                  color: idx === activeIdx ? '#9a3412' : '#78350f',
                  borderRight: idx < paths.length - 1 ? '1px solid #fed7aa' : 'none',
                }}
              >
                Path {idx + 1}
              </button>
        ...
        {activePath && (
          <div style={{ padding: '6px 10px', ...(btIdx >= 0 ? { margin: 4, border: '1.5px dashed #dc2626', borderRadius: 6 } : {}) }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: btIdx >= 0 ? '#b91c1c' : '#c2410c', marginBottom: 3, lineHeight: 1.3 }}>
              {activePath.label}
            </div>
            {(activePath.courses ?? []).map((c) => (
              <div key={c} style={{ fontSize: 11, color: '#374151', padding: '1px 0' }}>&#x2022; {c}</div>
            ))}
            {(activePath.additionalRequirements ?? []).map((req, i) => (
              <div key={i} style={{ fontSize: 11, color: '#374151', fontStyle: 'italic', padding: '1px 0' }}>+ {req}</div>
            ))}
            {creditHours != null && (
              <div style={{ marginTop: 4, fontSize: 10, color: '#9ca3af' }}>{creditHours} hrs total</div>
            )}
          </div>
        )}
      </div>
...
function buildReactFlowGraph(graph, semesters, colOverride = null, selectedTrackId = null) {
  if (!graph?.nodes?.length) return { nodes: [], edges: [] };

  // Track-pool filtering — single-page multi-track majors (e.g. BMEN) show only the
  // student's selected track pool (plus catch-all pools); other track pools hidden.
  ...
  // Code + match-alias → RF node id lookup
  const codeToId = new Map();
  for (const n of nodeList) {
    const rfId = n.id ?? n.code;
    codeToId.set(String(n.code ?? rfId).toUpperCase(), rfId);
    // Don't alias an elective/pool node's option courses to the pool — aliasing them draws
    // spurious prereq edges into the pool. Choice (cycling) + cross-listed nodes still alias.
    if (n.requirementType === 'elective') continue;
    for (const m of n.matches ?? []) codeToId.set(String(m).toUpperCase(), rfId);
  }

  // Per-target OR-group coloring: each prereqGroups entry with ≥2 interchangeable
  // alternatives gets a color index. Keyed "TARGETCODE|SOURCECODE" for the edge builder.
  ...
  // Infer columns for nodes that have no semesterColumn
  ...
  // Group nodes by column id string
  const colMap = new Map();
  for (const n of nodeList) {
    const rfId = n.id ?? n.code;
    let col;
    if (colOverride) {
      // Smart schedule: use numeric semester number as column key
      const semNum = colOverride.get(rfId) ?? 1;
      col = `sem-${semNum}`;
    } else {
      col = n.semesterColumn;
      if (col == null) {
  ...
  // Sort: sem-N (smart) → numeric; named cols (catalog) → lexicographic; inferred last
  const cols = [...colMap.keys()].sort((a, b) => {
    const aSem = a.startsWith('sem-');
    const bSem = b.startsWith('sem-');
    const aInf = a.startsWith('__inf_');
    const bInf = b.startsWith('__inf_');
    if (aSem && bSem) return Number(a.slice(4)) - Number(b.slice(4));
    if (!aInf && !bInf && !aSem && !bSem) return a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' });
  ...
  for (let ci = 0; ci < cols.length; ci++) {
    ...
    // Column header label
    let label;
    if (col.startsWith('sem-'))    label = `Semester ${col.slice(4)}`;
    else if (col.startsWith('__inf_')) label = `Semester ${Number(col.slice(6)) + 1}`;
    else                           label = columnIdToLabel(col);
    rfNodes.push({
    ...
    list.forEach((n) => {
      ...
      // Multi-option choice: expose each option + student status for cycling. Compound
      // (bundle) choices cycle by ALTERNATIVE (keyed "PHYS 207+PHYS 227"); plain choices
      // cycle by single course code.
      ...
      const y = yCursor;
      // Reserve vertical room based on the node's rendered height so tall nodes
      // (compound "choose one" lists each option on its own line; path_option has a
      // path selector + course lists) don't overlap the next node in the column.
  ...
  // Build edges — store source/target for later highlight use
  const rfEdges  = [];
  const edgeSeen = new Set();
  // Track how many edges share the same source+target pair so we can offset duplicates
  ...
  // Track per-pair index so each edge within a pair gets a unique curvature offset
  ...
  for (const e of graph.edges ?? []) {
    ...
    // Spread curvature so multiple edges between the same nodes don't overlap:
    // single edge → 0.25 (default), multiple → spread around 0.25
...
// Full transitive chain — all ancestors and all descendants via BFS
...
// "Choose one" requirements aren't unavoidable — a student picks the CHEAPEST
// option. Resolve each choice node to its minimum-cost option(s) before measuring
// the bottleneck: drop prereq edges of strictly-longer options (keep ties).
function resolveChoiceMinima(edges) {
  ...
  const drop = new Set();
  const multiOptionChoices = new Set(); // choice nodes with ≥2 equally-short options
...
// through the prereq DAG. Prereq edges weigh 1 (later semester); coreq edges weigh
// 0 (same semester). Every maximum-weight path is an "equivalent route" (e.g. MATH
// 171→172→221→409→437 vs 171→172→300→323→437); the UI pages through them one at a
// time. Pass edges through resolveChoiceMinima() first so "choose one" requirements
// only count their cheapest option. Returns routes + length. Cycle-safe.
function computeBottleneckRoutes(edges, maxRoutes = 40) {
  ...
  if (length === 0) return { routes: [], length: 0 };
  ...
  for (const e of edges) {
    ...
    if (criticalNodes.has(e.source) && criticalNodes.has(e.target) && f(e.source) + w + h(e.target) === length) {
      ...
      if (!arr.includes(e.target)) arr.push(e.target); // dedup parallel edges (course that is a prereq of two paths of one "choose one")
    }
  }
  const routes = [];
...
function Legend() {
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, padding: '8px 14px', background: 'var(--color-surface,#fff)', borderBottom: '1px solid var(--color-border)', alignItems: 'center' }}>
      ...
      {/* Full-width flex break: drop Prerequisite/Corequisite onto their own second line. */}
      <div style={{ flexBasis: '100%', height: 0 }} />
      {[
        { label: 'Prerequisite', dashed: false },
        { label: 'Corequisite',  dashed: true  },
      ].map((it) => (
        <div key={it.label} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12 }}>
          <svg width={30} height={14} style={{ flexShrink: 0 }}>
            <line x1={1} y1={7} x2={23} y2={7} stroke="#6b7280" strokeWidth={2}
              strokeDasharray={it.dashed ? '5,3' : undefined} />
            <polygon points="20,4 28,7 20,10" fill={it.dashed ? 'none' : '#6b7280'} stroke="#6b7280" strokeWidth={it.dashed ? 1 : 0} />
          </svg>
          <span style={{ color: '#374151' }}>{it.label}</span>
        </div>
      ))}
      <div style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--color-text-muted)' }}>
        Border &amp; code = subject color &nbsp;&middot;&nbsp; Scroll to zoom &middot; drag to pan
      </div>
    </div>
  );
}
...
// ---------------------------------------------------------------------------
// Inner component — uses useReactFlow (must be a child of <ReactFlow>)
// ---------------------------------------------------------------------------
function FlowController() {
  ...
  useEffect(() => {
    function handleKeyDown(evt) {
      ...
      if ((evt.key === 'f' || evt.key === 'F') && !evt.shiftKey && !isEditing) {
...
// Main page component
// ---------------------------------------------------------------------------
export default function StudentFuturePlanning() {
  ...
  // 1 = direct neighbors only (click), 2 = full transitive chain (press A)
  const [selectionDepth, setSelectionDepth] = useState(1);
  ...
  // Choice-node cycling: nodeId → active option index, plus metadata (counts + codes).
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const loadedSems = planRes.data.semesters ?? [];
        ...
        if (mounted) setError(err?.response?.data?.error ?? 'Failed to load planner data.');
      } finally {
  ...
  // Bottleneck summary (courses / semesters) for the footer — derived from
  // reactive state (nodes carry the active option index; edges carry topology),
  // so no ref reads during render.
  const bottleneckInfo = useMemo(() => {
    if (!bottleneckActive) return null;
    const { routes, length } = computeBottleneckRoutes(resolveChoiceMinima(edges).edges);
    const count = routes.length;
    if (count === 0) return { courses: 0, semesters: 0, count: 0, index: 0 };
    const idx = ((bottleneckRouteIdx % count) + count) % count;
    return { courses: new Set(routes[idx]).size, semesters: length + 1, count, index: idx + 1 };
  }, [bottleneckActive, edges, bottleneckRouteIdx]);
  ...
  // Apply highlight roles whenever selection or the active choice option changes
  useEffect(() => {
    ...
    if (bottleneckActive) {
      ...
      // chains are considered. Show ONE equivalent route at a time (←/→ to page).
      const { edges: resolved } = resolveChoiceMinima(edges);
      const { routes } = computeBottleneckRoutes(resolved);
      const count = routes.length;
      ...
      setNodes((nds) => nds.map((n) => {
        ...
        return { ...n, data: { ...n.data, highlight: onRoute ? 'bottleneck' : 'dimmed', bottleneckOption: onRoute ? (routeOption.get(n.id) ?? null) : null } };
      }));
    ...
    setNodes((nds) => {
      const activeTopo = edges.filter(edgeActive);
      if (!selectedNodeId) {
      ...
      return nds.map((n) => {
        if (n.type === 'columnHeader') return n;
        let hl;
        if (n.id === selectedNodeId)   hl = 'selected';
        else if (prereqs.has(n.id))    hl = 'prereq';
        else if (dependents.has(n.id)) hl = 'dependent';
        else                           hl = 'dimmed';
        return { ...n, data: { ...n.data, highlight: hl } };
      });
    });
    setEdges((eds) => {
      const activeTopo = eds.filter(edgeActive);
      if (!selectedNodeId) {
        return eds.map((e) => {
  ...
  const onNodeClick = useCallback((_evt, node) => {
    ...
    if (node.data?.requirementType === 'elective' && !node.data?.code) return;
    setBottleneckActive(false);
    setSelectionDepth(1);
  ...
  // 'A' key: press to toggle full chain view / deselect
  useEffect(() => {
    function handleKeyDown(evt) {
      const tag = document.activeElement?.tagName;
      const isEditing = tag === 'INPUT' || tag === 'TEXTAREA' || document.activeElement?.isContentEditable;
      // Escape always deselects, even from inputs
      if (evt.key === 'Escape') {
        setSelectedNodeId(null);
        setSelectionDepth(1);
        setBottleneckActive(false);
      ...
      if ((evt.key === 'b' || evt.key === 'B') && !isEditing) {
      ...
      if (bottleneckActiveRef.current && !isEditing && (evt.key === 'ArrowRight' || evt.key === 'ArrowLeft')) {
        evt.preventDefault();
        setBottleneckRouteIdx((i) => i + (evt.key === 'ArrowRight' ? 1 : -1));
        return;
      }
      if ((evt.key !== 'a' && evt.key !== 'A') || isEditing) return;
      setSelectedNodeId((prev) => {
        if (!prev) return prev; // nothing selected, do nothing
        if (selectionDepthRef.current === 1) {
          setSelectionDepth(2); // expand to full chain
          return prev;
        }
        // Already at full chain → deselect
        setSelectionDepth(1);
        return null;
      });
    }
  ...
  return (
    <>
      <div className="page-header" style={{ marginBottom: 0 }}><h1>Degree Flowchart</h1></div>
      ...
      {!loading && !error && !hasProgramId && (
        <div className="empty-state" style={{ marginTop: 16 }}>
          <img src="/icon-flowchart.png" alt="" style={{ width: 52, height: 52, objectFit: 'contain', opacity: 0.35, marginBottom: 8 }} />
          <h3>No degree program selected</h3>
          <p>Go to the <a href="/student/degree-planner">Degree Planner</a> and select your primary major to see the degree flowchart.</p>
        </div>
      )}

      {!loading && !error && hasProgramId && !hasGraph && (
        <div className="empty-state" style={{ marginTop: 16 }}>
          <img src="/icon-degree-planner.png" alt="" style={{ width: 52, height: 52, objectFit: 'contain', opacity: 0.35, marginBottom: 8 }} />
          <h3>No graph data available</h3>
          <p>A detailed degree flowchart is not yet available for your program (<strong>{academicProfile.primaryMajor.programId}</strong>). Check back after your program director sets it up.</p>
        </div>
      )}

      {!loading && !error && hasProgramId && hasGraph && graph.availableTracks?.length > 0 && !selectedTrackId && (
        <div className="empty-state" style={{ marginTop: 16 }}>
          <img src="/icon-flowchart.png" alt="" style={{ width: 52, height: 52, objectFit: 'contain', opacity: 0.35, marginBottom: 8 }} />
          <h3>Select your track first</h3>
          <p>
            Your major has multiple tracks. Go to the{' '}
            <a href="/student/degree-planner">Degree Planner</a>, open your Academic Profile,
      ...
      {!loading && !error && hasProgramId && hasGraph && !(graph.availableTracks?.length > 0 && !selectedTrackId) && (
        <div
          ...
          style={isFullscreen ? {
            ...
            marginTop: 12, border: '1px solid var(--color-border)', borderRadius: 8, overflow: 'hidden', background: '#f9fafb',
          }}
        >
          ...
          <div style={{ padding: '8px 14px', background: 'var(--color-surface,#fff)', borderTop: '1px solid var(--color-border)', fontSize: 12, color: 'var(--color-text-muted)', display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
            <span>
              Click a course → <strong>direct neighbors</strong>.&nbsp;
              Press <kbd style={{ fontSize: 11, padding: '1px 4px', border: '1px solid #d1d5db', borderRadius: 3, background: '#f9fafb' }}>A</kbd> → <strong>full chain</strong>.&nbsp;
              Press <kbd style={{ fontSize: 11, padding: '1px 4px', border: '1px solid #d1d5db', borderRadius: 3, background: '#f9fafb' }}>A</kbd> again or click background to deselect.&nbsp;
              Press <kbd style={{ fontSize: 11, padding: '1px 4px', border: '1px solid #d1d5db', borderRadius: 3, background: '#f9fafb' }}>F</kbd> → <strong>fit view</strong>.&nbsp;
              Press <kbd style={{ fontSize: 11, padding: '1px 4px', border: '1px solid #d1d5db', borderRadius: 3, background: '#f9fafb' }}>Shift+F</kbd> → <strong>{isFullscreen ? 'exit fullscreen' : 'fullscreen'}</strong>.&nbsp;
              Press <kbd style={{ fontSize: 11, padding: '1px 4px', border: '1px solid #d1d5db', borderRadius: 3, background: '#f9fafb' }}>B</kbd> → <strong>degree bottleneck</strong>.
            </span>
            {bottleneckActive && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '2px 8px', borderRadius: 999, fontSize: 11, fontWeight: 600, background: '#fee2e2', color: '#b91c1c', border: '1px solid #fecaca' }}>
                Degree bottleneck{bottleneckInfo && bottleneckInfo.count > 0 ? ` — ${bottleneckInfo.courses} courses, ~${bottleneckInfo.semesters} semesters${bottleneckInfo.count > 1 ? ` · route ${bottleneckInfo.index} of ${bottleneckInfo.count} (←/→ to page)` : ''}` : ''}
              </span>
            )}
            {!bottleneckActive && selectedNodeId && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '2px 8px', borderRadius: 999, fontSize: 11, fontWeight: 600,
                background: selectionDepth === 1 ? '#eff6ff' : '#f3e8ff',
                color:      selectionDepth === 1 ? '#1d4ed8' : '#7c3aed',
                border:     `1px solid ${selectionDepth === 1 ? '#93c5fd' : '#c4b5fd'}`,
              }}>
            ...
            {selectedNodeId && (
              <span style={{ marginLeft: 'auto', display: 'flex', gap: 12, alignItems: 'center' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '2.5px solid #f59e0b', background: '#fff' }} />
                  <span style={{ color: '#374151' }}>Selected</span>
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '2.5px solid #3b82f6', background: '#fff' }} />
                  <span style={{ color: '#374151' }}>Prerequisites</span>
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '2.5px solid #a855f7', background: '#fff' }} />
                  <span style={{ color: '#374151' }}>Dependents</span>
                </span>
              </span>
            )}
          </div>
